    ...
    public class PasswordGenerator
    {
        ...
        public Roles ParseRole(string roleText)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Выберите роль для генерации соответствующего пароля!", "Ошибка!", MessageBoxButton.OK, MessageBoxImage.Error);
                    return Roles.Def;  
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("Произошла ошибка: " + ex.Message, "Упс!", MessageBoxButton.OK, MessageBoxImage.Error);
                throw;  
            }
        }
